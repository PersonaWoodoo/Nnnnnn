...
# -------- Конфигурация --------
BOT_TOKEN = os.getenv('BOT_TOKEN', '8700350538:AAHg9xfB6n_EK77xLoRoOPL6xEHjNZvAWXg')
...
if not BOT_TOKEN:
    ...
    print('Добавьте BOT_TOKEN в файл .env или пропишите в коде')
    exit()
...
def check_actions(check_code: str, is_creator: bool = False) -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(
            text='▶️ Активировать',
            url=f'https://t.me/LUDO_GMP_BOT?start=check_{check_code}'
        )
    )
    ...
    buttons.append(InlineKeyboardButton(
        text='📤 Поделиться',
        switch_inline_query=f'🎫 Чек {check_code}\nАктивируй: https://t.me/LUDO_GMP_BOT?start=check_{check_code}'
    ))
    buttons.append(InlineKeyboardButton(
        text='📨 Переслать',
...
async def cmd_start(message: Message):
    ...
    if not user:
        ...
        # Если есть реферер, сообщаем о бонусе
        if referrer_id:
            await message.answer(
                ...
                f'Ваш реферер получил +{REFERRAL_BONUS} GMP\n\n'
...
async def process_check_amount(message: Message, state: FSMContext):
    ...
    try:
        ...
        check_message = (
            ...
            f'https://t.me/LUDO_GMP_BOT?start=check_{result["code"]}\n\n'
            f'📢 Наш канал: {CHANNEL_LINK}'
        )
...
async def referral_menu(callback: CallbackQuery):
    ...
    text = (
        ...
        f'Ваша реф-ссылка:\n'
        f'https://t.me/LUDO_GMP_BOT?start=ref_{user_id}\n\n'
    ...
    builder.row(
        InlineKeyboardButton(
            ...
            switch_inline_query=f'Присоединяйся к GMP! Получи бонус: https://t.me/LUDO_GMP_BOT?start=ref_{user_id}'
        )
    )
    builder.row(
        InlineKeyboardButton(text='🔙 Назад', callback_data='back_to_menu')
...
async def admin_deposits(callback: CallbackQuery):
    ...
    if not deposits:
        ...
        for dep in deposits[:10]:
            text += f'#{dep[0]} | Пользователь: {dep[1]} | {dep[3]}\n'
