...
def check_actions(check_code: str, is_creator: bool = False) -> InlineKeyboardMarkup:
    ...
    buttons.append(InlineKeyboardButton(
        text='📤 Поделиться',
        switch_inline_query=f'🎫 Чек {check_code}\nАктивируй: https://t.me/GMP_TASKS_BOT?start=check_{check_code}'
    ))
    buttons.append(InlineKeyboardButton(
        text='📨 Переслать',
...
async def cmd_start(message: Message):
    ...
    if not user:
        ...
        await message.answer('🏠 Главное меню:', reply_markup=main_menu_inline(user_id))
    else:
        await message.answer(
            f'🏠 Главное меню\n\n💰 Баланс: {user["balance"]} GMP\n📢 Наш канал: @GMP_TASKS_BOT',
            reply_markup=get_main_keyboard(user_id)
        )
        await message.answer('Выберите действие:', reply_markup=main_menu_inline(user_id))
...
async def back_to_menu(event):
    ...
    if isinstance(event, CallbackQuery):
        ...
        await event.answer('Выберите действие:', reply_markup=main_menu_inline(user_id))
...
async def process_check_amount(message: Message, state: FSMContext):
    ...
    try:
        ...
        check_message = (
            ...
            f'https://t.me/GMP_TASKS_BOT?start=check_{result["code"]}\n\n'
...
async def admin_confirm(callback: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        if type == 'deposit':
            ...
            # Здесь нужно сохранить состояние для ввода суммы
            
            await callback.message.edit_text('✅ Депозит подтвержден! (сумму нужно ввести отдельно)')
